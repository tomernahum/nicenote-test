...
- document secret keys   (not known by server)
    - read key
    - write key (used for signing write attempts)
        - can have multiple for different groups of actions (eg public and private)
    - permissions write key (used for signing writes to permissions)
        - maybe reads to permissions should be encrypted seperately
...
- add someone to a permission of the document:
    - client will get their secret key package
    - decrypt it to get the appropriate secret key
    - get the public key of the person they want to add
        (can send this out of band)
    - encrypt the appropriate key with their public key
...
- change document content
    ...
    - client signs this change with the edit secret key
    ...
    - other clients also verify the change was from someone with the correct signing key, in case the server turned evil
        - if it's not, clients reject the edit and log an error or something
